...
def fixed_t_commsum_T1(terms, couplings, t): # NOTE: OBSOLETE, DO NOT USE
    """
...
def pauli_string_to_matrix(pauli_string, N):
    """
    Convert a Pauli string (e.g. 'XYZ') into either its standard or sparse matrix representation, depending on its size.
    Invalid input returns a zero sparse matrix and raises a warning.
    """
    dim = 2 ** N

    if N < N_THRESHOLD_SPARSE:
        if pauli_string == '': 
        ...
        for p in pauli_string[1:]: matrix = np.kron(matrix, PAULI[p]) 

        return matrix
    else:
        if pauli_string == '':
            warnings.warn("Empty Pauli string provided.")
            return csr_matrix((dim, dim), dtype=complex)

        for p in pauli_string:
            if p not in PAULI_SPARSE:
                warnings.warn(f"Invalid Pauli operator '{p}' in string '{pauli_string}'.")
                return csr_matrix((dim, dim), dtype=complex)

        matrix = PAULI_SPARSE[pauli_string[0]]
        for p in pauli_string[1:]: matrix = kron(matrix, PAULI_SPARSE[p], format="csr")

        return matrix


def eval_additive_error_term_T1(error_terms, couplings, N): 
    """
    ### Formula for a'_1(N,G)
    ...
    for spec_norm_term in tqdm(error_terms, position=0):
        error_contribution = 0

        for pstring, terms in tqdm(spec_norm_term.items(), position=1, leave=True, total=len(spec_norm_term), bar_format="{l_bar}\033[92m{bar}\033[0m{r_bar}"):
            coeff = 0

            for term in terms: 
                if term[1] != '': coeff += term[0] * eval_couplings(term[1], couplings)

            error_contribution += coeff*pauli_string_to_matrix(pstring, N)

        if N < N_THRESHOLD_SPARSE:
            err_vl += np.max(np.linalg.svd(error_contribution, compute_uv=False))
        else:
            err_vl += svds(error_contribution, k=1, which="LM", return_singular_vectors=False)[0]

    return 0.5 * err_vl

def eval_additive_error_term_T1_triang(couplings, N):
    """
    couplings (dict): {"g12": 0.1, "g13": 0.2, ...}
    t (float): time length of simulation
    order (int): order of Trotter formula

    return: value of first order Trotter error for a system of N qubits for a given set of couplings
    """
    ...
    for idx1 in range(1,N+1):
        for idx2 in range(idx1+1, N+1):
            for idx3 in range(1,N+1):
                for idx4 in range(idx3+1,N+1):
                    if (
                    (idx1 == idx3 and idx2 != idx4) or
                    (idx2 == idx3 and idx1 != idx4)
                    ): err_vl += eval_couplings(f"(g{idx1}_{idx2})(g{idx3}_{idx4})", couplings)
...
def eval_additive_error_term_T2(error_terms, couplings, t): # TODO
    """
    error_terms  (list of dicts): [{"XYZ": [[-0.5j, "(g12)(g23)"], ...], ...}, {"XYZ": [[0.5j, "(g12)(g13)"], ...], ...}]
    couplings (dict): {"g12": 0.1, "g13": 0.2, ...}
    t (float): time length of simulation
    ...
    return 0.5 * err_vl * t ** 2
    
    err_vl = 0

    for comm_dict in error_terms:
    ...
    return err_vl * t**3 / 12

def eval_additive_error_term_T2_triang(couplings, N):
    """
    couplings (dict): {"g1_2": 0.1, "g1_3": 0.2, ...}

    return: value of second order Trotter error bound (triangle inequality)
    """
...
def fp_couplings(N, peakness, mu): # fp = forward peaked
    ...
    for i in range(N):
        for j in range(i+1, N):
            angle_dic[f"g{i+1}_{j+1}"] = theta_nu(N, i, j, peakness)
            coupl_dic[f"g{i+1}_{j+1}"] = mu * (1-np.cos(theta_nu(N, i, j, peakness))) / N
