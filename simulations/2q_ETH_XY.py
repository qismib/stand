...
# sigma_{x,y,z}_{1,2} for the XY interaction as a function of the 
...
def main():
    ...
    # system Hamiltonian: XY interaction
    H_XY = 0.5*J * (sx1*sx2 + sy1*sy2)
    ...
    secnd.plot(phase_angle, ey1, 'r--', label=r"$\langle\sigma_y^1\rangle$") # TODO not matching paper
    secnd.plot(phase_angle, ey2, 'b--', label=r"$\langle\sigma_y^2\rangle$")
    third.plot(phase_angle, ez1, 'r--', label=r"$\langle\sigma_z^1\rangle$")
    third.plot(phase_angle, ez2, 'b--', label=r"$\langle\sigma_z^2\rangle$")

    fig.suptitle("Expectation values of Pauli under XY.")
    ...
    fig.savefig(f'..\\plots\\2q_ETH_XY\\expvl_Pauli.pdf', dpi=300)
    plt.close(fig)
